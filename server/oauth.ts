...
async function approveAuthorize(
  ...
  if (accountId === null)
    return oauthError(
      ...
      401,
      'access_denied',
      'log in to your BharatVerse account first',
    );
...
<title>Authorize ${escapeHtml(o.client)} · BharatVerse</title><meta name="robots" content="noindex">${PAGE_STYLE}</head>
<body><main>
  ...
  <p style="font-size:13px;color:#7c6f4e">Signed in via your BharatVerse web session.</p>
</main>
<script>
  var REQ = ${data};
  ...
  document.getElementById('allow').onclick=function(){
    var t=token();
    if(!t){ document.getElementById('msg').textContent='Please log in to BharatVerse first, then reopen this link.'; return; }
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Link a device · BharatVerse</title><meta name="robots" content="noindex">${PAGE_STYLE}</head>
<body><main>
  <h1>Link a device</h1>
...
<script>
  ...
  document.getElementById('ok').onclick=function(){
    var t=token(); if(!t){ document.getElementById('msg').textContent='Please log in to BharatVerse first.'; return; }
