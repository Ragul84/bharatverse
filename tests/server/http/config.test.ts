...
describe('loadConfig', () => {
  ...
  it('validates REALMS: a usable entry passes, a garbage list throws, same-origin/unset pass', () => {
    expect(() =>
      loadConfig({ ...MIN_ENV, REALMS: 'Bharat=https://claudemoon.example.com=Normal' }),
    ).not.toThrow();
    // An empty url is the same-origin realm parseRealms keeps.
    expect(() => loadConfig({ ...MIN_ENV, REALMS: 'Bharat=' })).not.toThrow();
    expect(() => loadConfig({ ...MIN_ENV })).not.toThrow();
    // No '=' at all, or a non-bare url, yields zero usable entries.
    expect(() => loadConfig({ ...MIN_ENV, REALMS: 'Bharat' })).toThrow(/REALMS/);
    expect(() => loadConfig({ ...MIN_ENV, REALMS: 'Bharat=not-a-url' })).toThrow(/REALMS/);
    // A credentialed url is not a bare origin either (same isBareOrigin rule).
    expect(() =>
      loadConfig({ ...MIN_ENV, REALMS: 'Bharat=https://u:p@claudemoon.example.com' }),
    ).toThrow(/REALMS/);
  });
