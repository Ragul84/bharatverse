...
class FakeDb implements SocialDb {
  ...
  addChar(id: number, name: string, cls = 'warrior', level = 10, realm = 'Bharat'): void {
...
describe('resolveRealm', () => {
  it('accepts realm-style display names', () => {
    expect(resolveRealm('Bharat')).toBe('Bharat');
  ...
  it('falls back to the default for empty or invalid names', () => {
    expect(resolveRealm(undefined)).toBe('Bharat');
    expect(resolveRealm('')).toBe('Bharat');
    expect(resolveRealm('x'.repeat(25))).toBe('Bharat');
    expect(resolveRealm('drop;table')).toBe('Bharat');
  });
});
