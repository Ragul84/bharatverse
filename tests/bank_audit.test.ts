...
// the fields a case cares about. Every row is 'personal' with realm Bharat.
...
describe('auditBank', () => {
  ...
  it('reconciles ledger activity against an EMPTY bank when the state has none', () => {
    ...
    const findings = auditBank({
      ...
      characters: [
        { id: 50, realm: 'Bharat', state: null }, // NULL state, ledger activity
        { id: 51, realm: 'Bharat', state: { pos: { x: 0, z: 0 } } }, // state without bank
        { id: 52, realm: 'Bharat', state: null }, // pre-bank, no activity: skipped
      ],
    });
...
describe('formatReport', () => {
  ...
  it('renders one FINDING line per anomaly plus the per-container summary', () => {
    ...
    expect(report).toContain(
      'FINDING: container personal: realm Bharat: character 9: negative_net: net -3 of wolf_fang',
    );
